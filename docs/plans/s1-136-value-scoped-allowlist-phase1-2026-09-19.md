...
| Finding | Disposition |
|---|---|
| Red-team F1, Issue 243 MED | Fixed. The rejected-entry line shows the allowlist entry's path and pattern id percent-encoded and clipped to a fixed length (a lone percent sign for a non-string or empty value). Test `oss01-rejected-entry-line-never-carries-a-shell-metacharacter-from-the-allowlist-file`, red first, with a positive control that the raw-echo shape executes here. `REJECTED-ALLOWLIST-FILE` carries no file text. Other places allowlist-file text is printed: the blocking-match and ALLOWLISTED lines (pre-existing, Issue 241) and the problem lines of `allowlist-tool verify` (a developer-run tool, outside this round's scope, named in the ADR residual row). |
