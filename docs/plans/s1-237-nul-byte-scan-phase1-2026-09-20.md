...
**Status: BUILT** (commits S0, C1, C2 and C3 on branch `fix/s1-237-nul-byte-scan`; review round 1 wording corrections applied). This document is the plan as written before the build; one prediction changed in the build: section 7 and step C2 expect three old tests to fail once the source changes, and measured only the window test did (decisions row (h)). Story: Issue 237 (found by design-challenger, S-B2 pre-build round 1, attack A1). Branch `fix/s1-237-nul-byte-scan`, cut from origin/master e42a54f (S-B2 merged). Tier PROPOSED: **CRITICAL** (the Manager ratifies; not yet persisted to `docs/.maat-state.json`, that write waits for the ratification).
...
- **Instrument, not hand count (CLAUDE.md completeness rule).** Whitespace-sensitive tokens are enumerated by running a tokenizer over `SECRET_PATTERNS[].regex.source` (test R237-7; the preview run found: `aws-secret-access-key` two `\s*` separators; `private-key-block` the `[\s\S]` any-character idiom; `generic-password-assignment` two `\s*` separators and one `\s` inside the negated value class). Classification by position:
  ...
  - separator (`\s*` between the name, the operator and the value): treating 0xA0 as whitespace here matches more and hides no data. Four tokens. **Not changed** (refinement of the ruling's letter, question 1): converting them would stop matching a latin1 file whose separator is a lone 0xA0, a new evasion, for no gain. Pinned by R237-9.
  - the `[\s\S]` idiom: any character, independent of the whitespace definition. One token. Not changed.
- **Widening (why the blast radius is bounded): measured, not proved.** Removing the skip and widening the value class add matches. A value that matched before contained no 0xA0 (the class excluded it and the closing quote had to follow), so, taken alone, it still matches. That does not make the new result set a superset of the old one. Under leftmost non-overlapping matching, a widened value can run past a 0xA0, close on a later quote, and consume an adjacent assignment that the old class matched by itself (found by the red-team review of 2026-09-20, finding 4; the consuming match still blocks, and 0 of 50 allowlist entries use this pattern). Measured on this repository: 0 of 1097 history blobs are affected and the old and new triple sets are equal (R237-11; the preview found 0). Both changes still fail closed for the gate as a whole: the text that swallows an assignment is itself a match.
...
| A future legitimate binary asset that produces a match | Not a chance-match rate: random bytes match only occasionally (50 MB gave 2), but real executables and fonts carry embedded emails and hostnames and match far more often (app-security review of 2026-09-20, finding 1, on the reviewer's own machine and a foreign corpus, illustrative: about half of the executables and fonts sampled had at least one match, 0 of 39 images did). Handled by the existing value-scoped allowlist and no new mechanism (Q1): **each tracked binary asset version would need its own reviewed value-scoped grant** (a hash and a reason in a PR; the unlock line already prints the command; a regenerated asset changes the matched bytes and needs a new grant). **This repository tracks none today** (`git ls-files --eol` shows 0 files classified `i/-text`) and 0 of 1097 history blobs newly block (the spike) |
