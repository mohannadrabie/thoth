...
constraints:
  security:
    ...
    - "The fixture loader MUST throw on malformed input. The fixture path MUST resolve only from CLAUDE_PROJECT_DIR (the project root, then docs/qa/s5-central-classification.json) or the module-adjacent DEFAULT_FIXTURE_PATH; no environment variable MAY select an arbitrary fixture file (GitHub Issue #99), and the resolved path and its source MUST be recorded in halt-state."
  ...
  code:
    - "The one-time, human-directed removal of the exact-pin and expiry tests (AC1-a, AC1-b, AC1-c, S5-R2-N2, the isFixtureExpired tests, the expiresOn and ratifiedBy tests) is a human-approved exception to SE ADR-0010 (MUST NOT delete tests); it MUST NOT be cited to delete any other test."
---
...
- **MUST** keep the loader throwing on malformed input. **MUST** keep the fixture path resolving only from `CLAUDE_PROJECT_DIR` (then `docs/qa/s5-central-classification.json`) or `DEFAULT_FIXTURE_PATH` (`hooks/sessionstart-tool-enum.mjs`, `resolveFixtureLocation`), recording the resolved path and source in halt-state. **MUST NOT** let any environment variable select an arbitrary fixture file (Issue #99). `CLAUDE_PROJECT_DIR` is the project-root seam every hook already uses, not a fixture selector.
...
- The exact-content pins, the expiry tests, and the `ratifiedBy` tests were deleted on the human's directive (rule 7).
