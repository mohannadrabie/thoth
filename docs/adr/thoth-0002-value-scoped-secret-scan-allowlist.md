...
- A rejected entry is named in the blocking output (index, path, pattern id, reason class; never a value or a hash; the path and pattern id are contributor-authored, so they are percent-encoded and clipped to 120 code points, Issue 243, as are the entry fields in `allowlist-tool verify`'s problem lines), and an unreadable, unparseable or non-array file is named as a file-level rejection.
...
| A NUL byte in the first 8000 bytes made the scanner skip a blob, an allowlist entry or not | Closed (issue 237): the skip is removed. Every blob is decoded as latin1 and matched like any other, so a NUL byte hides nothing, and a binary that produces a match is handled by this ADR's value-scoped entry; no other exemption shape is added. That is not a rare, chance event for real binaries: executables and fonts carry embedded emails and hostnames, and about half of the executables and fonts the app-security review sampled (a foreign corpus, illustrative) had at least one match, against 0 of 39 images. This repository tracks no binary asset (`git ls-files --eol` shows 0 files classified `i/-text`) and 0 of 1097 history blobs newly block at this writing; each tracked binary asset version that matches would need its own reviewed value-scoped grant, and a regenerated asset needs a new one. Tests scan blobs with a NUL byte at offsets 0, 1, 7000, 7999, 8000 and 9000, and blobs built in several byte shapes (leading, interior and trailing NUL, NUL padding, high bytes, an image-header prefix, a run of every byte value), and assert none is skipped. What remains open is UTF-16 text (a byte order mark and interleaved NUL bytes), which still matches nothing under latin1 decoding, and any other encoding that interleaves or transforms the bytes (UTF-32 and base64 also yield 0 matches): the same residual class, tracked under issue 246. The gate also had a second decode miss, a UTF-8 character whose second byte is 0xA0 ended a password value early; that is fixed in the value class of the generic-password pattern, and no allowlisted value changed (the generator's verify mode and a fresh generate are unchanged). |
