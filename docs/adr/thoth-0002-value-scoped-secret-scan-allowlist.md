...
| A NUL byte in the first 8000 bytes made the scanner skip a blob, an allowlist entry or not | Closed (issue 237): the skip is removed. Every blob is decoded as latin1 and matched like any other, so a NUL byte hides nothing, and a legitimate binary that produces a match is handled by this ADR's value-scoped entry; no other exemption shape is added. A test scans blobs with a NUL byte at every position and asserts none is skipped. What remains open is UTF-16 text (a byte order mark and interleaved NUL bytes), which still matches nothing under latin1 decoding: issue 246. The gate also had a second decode miss, a UTF-8 character whose second byte is 0xA0 ended a password value early; that is fixed in the value class of the generic-password pattern, and no allowlisted value changed (the generator's verify mode and a fresh generate are unchanged). |
