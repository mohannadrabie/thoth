...
Thoth exists to make governance real, not to own an implementation. Three rules, and they are normative:
...
These are Claude Code primitives, not AGT. Set them; do not build them.

| Primitive | Covers |
|---|---|
| Exclusive tool-server allowlist | SUR-04 |
| Credential denial at the capability layer | CTN-10 |
| Permission rules, bypass-mode lockout, sideload lockout, version floor | INT-01, REL-02 partially |
| Filesystem and network isolation for shell subprocesses | ENV-13 partially, SUR-11 partially (subprocesses only, see G4) |
...
| **Pinned central policy resolution.** POL-09 | AGT's design exists (ADR-0029); the implementation does not |
...
| ENV-13 | P1 | Environment selectors shall cover local blast radius, not only remote systems. | Writes outside the repository, to specified paths, or to any git remote, are expressible as a forbidden environment |
...
| POL-09 | P0 | The central policy source shall be resolvable by immutable reference, and the resolved reference shall be recorded in every artifact. | A change under review cannot alter the policy that judges it. The reference is pinned, not floating |
...
| SUR-04 | P0 | Structured tool servers reachable by the session shall be governed by an exclusive allowlist, not an additive one. | A tool server outside the allowlist does not load. This is the only control covering tools that never touch a shell |
...
| SUR-11 | P0 | Where the runtime's isolation does not cover a class of process, that gap shall be named in the posture block and covered by a compensating control. | Processes running outside the runtime sandbox are disclosed, not assumed governed |
...
| ID | Pri | Requirement | Acceptance |
|---|---|---|---|
| INT-01 | P0 | Thoth's own configuration and registration shall be protected. | Minimum: policy files, gate manifest, distribution manifest, and the runtime settings that install them |
...
| ~~T10~~ | ~~Which of AGT's policy formats is the delivery format, and is the tiered YAML the authoring format generated into it? AGT ships four mutually incompatible shapes: legacy flat YAML, a string-expression dialect, the ACS Rego manifest, and the adapter's `default-policy.json`.~~ **ANSWERED 2026-08-26: the adapter's `default-policy.json` shape is the delivery format. Thoth's tiered YAML (`policy/profiles/*.yaml`, `policy/catalog/*.yaml`, `maat.yaml`) remains the authoring format, generated into it — POL-14's freshness check is the staleness-guard mechanism for that generation.** | POL-01, POL-02, POL-08, POL-14 | Evidence: T10's own note that the adapter's shape is the only one wired to an enforcement point actually used; `docs/spikes/2026-08-25-agt-docs-recheck.md` §3 — AGT's own `docs/v4-removal.md` schedules runtime `governance.yaml` folder resolution (the legacy flat-YAML candidate) for deletion in a future breaking release, eliminating it on documented vendor intent; the ACS Rego manifest is tied to the still-beta (`0.3.1`) Rust kernel, the same "watch, don't commit" posture T13 already takes |
...
|---|---|---|---|
| **M1** | **Protect the baseline** | CI-01, QA-01 to QA-06, **QA-13 to QA-16**, OSS-01, INT-01. Plus: an ADR proposing the §1.7 wrap architecture (thoth owns the verdict, AGT plugs in behind five named interfaces), proposed and reviewed, never self-accepted | Nothing. Start here |
