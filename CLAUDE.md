...
**Issue shape:** `bug` label + the matching `severity:high`/`severity:med` label (per `docs/issue-template.md`) + this project's Feature ID label (the requirement-group label — `pol`/`sur`/`int`/`ops`/`qa`/`ci`/`oss`, kept in sync with `REQUIREMENTS.md`'s own group headers per `docs/issue-template.md`'s frontmatter) if one applies, else the `chore` catch-all. **Milestone:** link the Issue's native GitHub Milestone at filing time whenever the finding is against code touched within the CURRENT ratified scope (`docs/.maat-state.json`'s `scope` field) — that home is unambiguous, not a guess, so there's nothing to hedge against. Leave Milestone unset only when the finding's proper milestone home is genuinely unclear (e.g., a bug surfaced against already-shipped, unrelated code with no obvious owning story) — that's the one case the "no Milestone at filing time" default exists for. (2026-09-07 postmortem: all of S5's Stage 3 review Issues, #90-#98, were filed with neither the Feature ID label nor the Milestone link, because `sur`/`ops` labels didn't exist yet and the rule as originally written had no unambiguous-scope carve-out — both fixed same-day.) Body stays short per `docs/issue-template.md`: one-line summary + a link to the dated report in `docs/reviews/` — never paste the report's finding prose into the Issue body.
...
- No changes to the sensitive areas above (policy enforcement/guard, evidence trail, secret scanning/CI, policy delivery) without a fresh dated review report in `docs/reviews/` (see the Policy delivery exception above).
