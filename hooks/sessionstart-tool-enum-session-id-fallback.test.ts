...
// fail-closed. The fix: both scripts now consult `process.env.CLAUDE_SESSION_ID` (a real,
// host-supplied environment variable Claude Code sets on every hook subprocess, confirmed via the
// official Claude Code hooks/Settings Reference documentation) BEFORE falling back to the shared
// literal.
//
// Covers:
//   (a) malformed/empty stdin + CLAUDE_SESSION_ID set in the environment -> the halt-state file is
//       written under the REAL session id, not unknown-session.json.
//   (b) end-to-end: userpromptsubmit-halt-relay.mjs (given the SAME degraded stdin + the SAME
//       CLAUDE_SESSION_ID) finds and blocks on that exact file -- the full fail-closed path works,
//       not just the write side.
//   (c) double failure (stdin degraded AND CLAUDE_SESSION_ID also unset/empty): both scripts still
//       fall back to the literal UNKNOWN_SESSION_ID = "unknown-session" exactly as before -- the
...
//       CLAUDE_SESSION_ID at all -- byte-identical to before this fix, proven by setting
//       CLAUDE_SESSION_ID to a DIFFERENT value than the real stdin session_id and asserting the
...
// --- (a)+(b): malformed stdin, CLAUDE_SESSION_ID set -- real session id used, relay finds it -----

test("Issue #96: malformed (non-JSON) stdin with CLAUDE_SESSION_ID set in the environment -- the halt-state file is written under the REAL session id, not unknown-session.json, and the relay (same env, same degraded stdin) finds and blocks on it", () => {
  const tree = makeFixtureTree("issue96-malformed-stdin-env-set");
  try {
    const realSessionId = fakeSessionId("issue96-real-session");
    const env = { ...fixtureEnv(tree), CLAUDE_SESSION_ID: realSessionId };
    ...
    // (CLAUDE_SESSION_ID) instead. The parse failure itself is what criterion 17's own top-level
    ...
    assert.equal(
      ...
      false,
      "expected NO unknown-session.json to be written when CLAUDE_SESSION_ID successfully resolves a real session id",
    );
...
test("Issue #96: EMPTY stdin (not malformed JSON, just blank) with CLAUDE_SESSION_ID set -- same fallback applies (session_id field is simply absent from the resulting {} payload)", () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(haltStatePath(tree, UNKNOWN_SESSION_ID)), false, "expected no unknown-session.json for blank stdin when CLAUDE_SESSION_ID resolves");
  } finally {
    tree.cleanup();
  }
});
...
test("Issue #96 double-failure (unregressed): malformed stdin AND CLAUDE_SESSION_ID unset/empty -- both scripts still fall back to the literal UNKNOWN_SESSION_ID bucket exactly as before, and the round-2 sticky-halt guard (never reconciled to set:false) still protects it", () => {
  const tree = makeFixtureTree("issue96-double-failure");
  try {
    // Explicitly force CLAUDE_SESSION_ID to an empty string -- deterministic regardless of the
    ...
    const env = { ...fixtureEnv(tree), CLAUDE_SESSION_ID: "" };
    ...
    assert.equal(
      ...
      true,
      `expected run A's genuinely-active condition to still land in the shared "unknown-session" bucket when BOTH stdin and CLAUDE_SESSION_ID fail; got ${JSON.stringify(afterA)}`,
    );
...
// --- (d): happy path is byte-identical -- CLAUDE_SESSION_ID is never consulted when stdin is fine -

test("Issue #96 happy-path guard: when stdin parses fine and carries a real session_id, CLAUDE_SESSION_ID is NEVER consulted, even when it names a DIFFERENT session id -- the stdin value always wins", () => {
  ...
  try {
    ...
    const env = { ...fixtureEnv(tree), CLAUDE_SESSION_ID: differentEnvSessionId };
    ...
    assert.equal(
      ...
      false,
      "expected NO halt-state file under CLAUDE_SESSION_ID's differing value -- the env fallback must never override a well-formed stdin session_id",
    );
